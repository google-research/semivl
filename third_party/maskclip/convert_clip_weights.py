...
def parse_args():
    ...
    parser.add_argument('--model', default='ViT16', choices=['RN50', 'RN101', 'RN50x4', 'RN50x16', 'RN50x64', 'ViT32', 'ViT16', 'ViT14'], help='clip model name')
...
if __name__ == '__main__':
    ...
    for key in state_dict.keys():
        if 'ViT' in args.model and prefix in key:
            ...
            if new_key == 'proj':
                ...
                result_model['state_dict']['proj.weight'] = state_dict[key].float().t()
                continue
    ...
    if args.backbone:
        torch.save(result_model, f'pretrained/clip2mmseg_{args.model}_clip_backbone.pth')
    else:
        all_model['clip'] = result_model['state_dict']
        torch.save(all_model, 'pretrained/clip2mmseg_{}_clip_weights.pth'.format(args.model))
